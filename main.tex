...
\begin{align}
  ...
  +
  h
  \Phi(t_n,y(t_n),f(t_n,y(t_n));h)
  +
  O(h^{p+1})
  &
  \implies \tau_{n+1}(h) &= O(h^p)
  \\
  y(t_{n+1})
  &=
  y(t_n)
  +
  h
  \tilde \Phi(t_n,y(t_n),f(t_n,y(t_n));h)
  +
  O(h^{p+2})
  &
  \implies \tilde \tau_{n+1}(h) &= O(h^{p+1})
.\end{align}
Suponiendo que $u_n\approx\tilde u_{n}\approx y(u_n)$, tenemos
\begin{align}
  \tau_{n+1}(h)
  &\approx \frac{y(t_{n+1})-u_n}{h} - \Phi(t_n,u_n,f(t_n,u_n);h) \\
  &= \frac{y(t_{n+1})-[u_n+h\Phi(t_n,u_n,f(t_n,u_n);h)]}{h}  \\
  &= \frac{y(t_{n+1})-u_{n+1}}{h}  \\
  &= \frac{1}{h}e_{n+1}
.\end{align}
Similarmente, para el método de orden $p+1$, podemos obtener
\begin{equation}
  \tilde\tau_{n+1}(h)
  \approx \frac{y(t_{n+1})-\tilde u_{n+1}}{h}
  = \frac{1}{h}\tilde e_{n+1}
.\end{equation}
Por otro lado,
\begin{align}
  \tau_{n+1}(h)
  &= \frac{1}{h}(y(t_{n+1})-u_{n+1}) \\
  &= \frac{1}{h}(y(t_{n+1})-\tilde u_{n+1}+\tilde u_{n+1}-u_{n+1}) \\
.\end{align}
